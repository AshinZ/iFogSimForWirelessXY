...
public class ModulePlacementEdgewards extends ModulePlacement{
	...
	private JSONArray placeMessage;
	...
	public ModulePlacementEdgewards(List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators,
		...
		placeMessage = preMessage.getJSONArray("placeMessage");
		createMessage = preMessage.getJSONArray("createMessage");
		mapModules();
		setModuleInstanceCountMap(getCurrentModuleInstanceNum());
	}

	public ModulePlacementEdgewards(List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators,
									Application application, ModuleMapping moduleMapping, JSONObject _jsonObject){
		this.setFogDevices(fogDevices);
		this.setApplication(application);
		this.setModuleMapping(moduleMapping);
		this.setModuleToDeviceMap(new HashMap<String, List<Integer>>());
		...
		preMessage = _jsonObject.getJSONObject("preMessage");
		placeMessage = preMessage.getJSONArray("placeMessage");
		createMessage = preMessage.getJSONArray("createMessage");
		mapModules();
		setModuleInstanceCountMap(getCurrentModuleInstanceNum());
	...
	protected void mapModules() {
		...
		for(int deviceId : getCurrentModuleMap().keySet()){
			for(String module : getCurrentModuleMap().get(deviceId)){
				createModuleInstanceOnDevice(getApplication().getModuleByName(module), getFogDeviceById(deviceId), createMessage);
			}
		}
	}
			...
			while(modulesToPlace.size() > 0){ // Loop runs until all modules in modulesToPlace are deployed in the path
				...
				if(upsteamDeviceId > 0){
					if(upsteamDeviceId==deviceId){
						...
						if(totalCpuLoad + getCurrentCpuLoad().get(deviceId) > device.getHost().getTotalMips()){
							...
							if(placeMessage != null) {
								JsonWriter.writePlaceMessage(placeMessage , moduleName , device.getName() , "need to shift module upstream");
							}
							...
							if(placeMessage != null) {
								JsonWriter.writePlaceMessage(placeMessage , moduleName , device.getName() , "can be created");
							}
						}
					}
				}else{
					...
					if(totalCpuLoad + getCurrentCpuLoad().get(deviceId) > device.getHost().getTotalMips()){
						...
						if(placeMessage != null) {
							JsonWriter.writePlaceMessage(placeMessage , moduleName , device.getName() , "not possible");
						}
					}
					else{
						//****//
						...
						if(placeMessage != null) {
							JsonWriter.writePlaceMessage(placeMessage , moduleName , device.getName() , "success");
						}
