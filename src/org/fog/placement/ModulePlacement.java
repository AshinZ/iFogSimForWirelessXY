...
public abstract class ModulePlacement {
	...
	protected boolean createModuleInstanceOnDevice(AppModule _module, final FogDevice device, JSONArray createMessage){
		AppModule module = null;
		if(getModuleToDeviceMap().containsKey(_module.getName()))
			module = new AppModule(_module);
		else
		...
		if(canBeCreated(device, module)){
			...
			if(createMessage != null) {
				JsonWriter.writeCreateMessage(createMessage , module.getName() , device.getName() , "success");
			}
			...
			System.err.println("Module "+module.getName()+" cannot be created on device "+device.getName());
			if(createMessage != null) {
				JsonWriter.writeCreateMessage(createMessage , module.getName() , device.getName() , "failed");
			}
			System.err.println("Terminating");
			return false;
		}
