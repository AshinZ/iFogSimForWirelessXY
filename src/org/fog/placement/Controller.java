...
public class Controller extends SimEntity{
	...
	public Controller(String name, List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators, JSONObject _jsonObject) {
		super(name);
		this.applications = new HashMap<String, Application>();
		setAppLaunchDelays(new HashMap<String, Integer>());
		setAppModulePlacementPolicy(new HashMap<String, ModulePlacement>());
		...
		runMessage = _jsonObject.getJSONObject("runMessage");
		resultMessage = _jsonObject.getJSONObject("resultMessage");
	}
	...
	private void printNetworkUsageDetails() {
		...
		if(resultMessage != null) {
			JsonWriter.writeTotal(resultMessage , NetworkUsageMonitor.getNetworkUsage()/Config.MAX_SIMULATION_TIME);
		}
	}
	...
	private void printCostDetails(){
		...
		JsonWriter.writeCost(resultMessage , getCloud().getTotalCost());
	}
	...
	private void printPowerDetails() {
		for(FogDevice fogDevice : getFogDevices()){
			...
			if(resultMessage != null) {
				JsonWriter.writeDevice(resultMessage.getJSONArray("device") , fogDevice.getName() , fogDevice.getEnergyConsumption());
			}
		}
	}
	...
	private void printTimeDetails() {
		...
		if(runMessage != null) {
			JsonWriter.writeExecTime(runMessage , (Calendar.getInstance().getTimeInMillis() - TimeKeeper.getInstance().getSimulationStartTime()));
		}
		System.out.println("=========================================");
		...
		for(Integer loopId : TimeKeeper.getInstance().getLoopIdToTupleIds().keySet()){
			...
			double delay = TimeKeeper.getInstance().getLoopIdToCurrentAverage().get(loopId);
			System.out.println(name + " ---> " + delay);
			JsonWriter.writeAppLoopDelay(runMessage.getJSONObject("appLoopDelay") , name , delay);
		}
		System.out.println("=========================================");
		...
		for(String tupleType : TimeKeeper.getInstance().getTupleTypeToAverageCpuTime().keySet()){
			...
			JsonWriter.writeTuple(runMessage.getJSONObject("tuple") , tupleType , TimeKeeper.getInstance().getTupleTypeToAverageCpuTime().get(tupleType));
		}
